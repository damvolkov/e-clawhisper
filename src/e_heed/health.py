...
from e_heed.shared.settings import AppConfig, STTBackend, TTSBackend
...
async def check_services(config: AppConfig) -> HealthResult:
    ...
    match config.stt.backend:
        case STTBackend.EVOICE:
            tasks.append(_probe_http(str(config.stt.evoice.url), "stt:evoice"))
    ...
    match config.tts.backend:
        case TTSBackend.EVOICE:
            tasks.append(_probe_http(str(config.tts.evoice.url), "tts:evoice"))
