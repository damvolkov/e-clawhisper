"""Session commands — stop / status / health / logs (daemon IPC + inspection)."""
...
app = cyclopts.App(name="session", help="Daemon management (IPC).")
...
def status() -> None:
    ...
    table = Table(title="eheed daemon status")
...
def health() -> None:
    ...
    from e_heed.health import check_services
...
def logs(*, lines: int = 50) -> None:
    ...
    os.execvp(
        "journalctl",
        ["journalctl", "--user-unit", "e-heed", "-f", "-n", str(lines), "--no-hostname", "-o", "short-iso"],
    )
