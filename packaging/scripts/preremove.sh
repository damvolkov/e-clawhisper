...
systemctl stop e-heed.service 2>/dev/null || true
systemctl disable e-heed.service 2>/dev/null || true
...
if [ -n "$REAL_UID" ] && [ -S "/run/user/$REAL_UID/bus" ]; then
    sudo -u "$REAL_USER" XDG_RUNTIME_DIR="/run/user/$REAL_UID" \
        systemctl --user stop e-heed.service 2>/dev/null || true
    sudo -u "$REAL_USER" XDG_RUNTIME_DIR="/run/user/$REAL_UID" \
        systemctl --user disable e-heed.service 2>/dev/null || true
fi
