...
def _make_pipeline(llm: LLMAdapter) -> TurnPipeline:
    ...
    return TurnPipeline(stt=stt, llm=llm, tts=tts, vad_config=vad_cfg)


async def _collect_sentences(pipeline: TurnPipeline, transcript: str) -> list[str]:
    queue: asyncio.Queue[str | None] = asyncio.Queue()
    await pipeline._stage_llm_to_sentences(transcript, queue)
...
async def test_producer_yields_sentences_from_chunks() -> None:
    """Simulate LLM streaming text_delta → sentence queue."""
    llm = AsyncMock(spec=LLMAdapter)

    async def _fake_send_message(text: str):
...
async def test_producer_empty_response() -> None:
    """Empty LLM response produces no sentences (only poison pill)."""
    llm = AsyncMock(spec=LLMAdapter)
